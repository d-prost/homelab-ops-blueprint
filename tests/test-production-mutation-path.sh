...
grep -Fq 'flock -n "$deployment_lock_fd"' "$deploy" || fail 'deployment lock must fail closed instead of waiting or running concurrently'
grep -Fq 'exec bash "$script_dir/deploy-stack.sh" "$1" --ref "$2"' "$rollback" || fail 'explicit rollback must reuse the guarded deployment path'
