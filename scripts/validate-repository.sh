...
if command -v ansible-playbook >/dev/null 2>&1; then
  ...
  HOMELAB_LAB_HOSTNAME=ci-example ansible-playbook -i ansible/inventory/lab/hosts.yml ansible/playbooks/preflight.yml -e stack_name=dozzle -e homelab_repo_root="$repo_root" --syntax-check
  HOMELAB_LAB_HOSTNAME=ci-example ansible-playbook -i ansible/inventory/lab/hosts.yml ansible/playbooks/deploy-stack.yml -e stack_name=dozzle -e homelab_release_commit=1111111111111111111111111111111111111111 -e homelab_repo_root="$repo_root" --syntax-check
fi
